...
export default function RentalForm({
  ...
  // ===============================
  // 4. WHEN DEVICE CHANGES → LOAD & AUTO-SELECT ACCESSORIES
  // ===============================
  useEffect(() => {
    if (!formData?.device_id) {
      setDeviceAccessories([]);
    ...
    if (selectedDevice && Array.isArray(selectedDevice.accessories)) {
      ...
      // Auto pre-select all accessories if empty (e.g. creating fresh rental)
      setFormData((prev) => {
        const isInitialOrEmpty =
          !prev.accessory_id || prev.accessory_id.length === 0;
        return {
          ...prev,
          accessory_id: isInitialOrEmpty
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        isEditMode
          ? "Rental parameters modified successfully."
          : "New rental master created successfully!"
      );
        ...
        <form onSubmit={handleFormSubmit}>
          <div className="mx-auto w-full max-w-[1450px] space-y-5 px-4 py-6 sm:px-6 lg:px-8">
            ...
            <section className={cardClass}>
              <div className="p-5 sm:p-6">
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                  {/* Device Model with Auto Accessories pre-selection */}
                  <div className="rounded-[16px] border border-[#E8DEFF] bg-[#FAF8FF] p-4">
                    <label className={labelClass}>
                      Device Model <span className="text-rose-500">*</span>
                    </label>
                    <select
                      ...
                      onChange={(e) => {
                        const selectedId = e.target.value;
                        const matchedDevice = deviceModels.find(
                          (dev) => Number(dev.device_id) === Number(selectedId)
                        );
                        const defaultAccessories =
                          matchedDevice &&
                          Array.isArray(matchedDevice.accessories)
                            ? matchedDevice.accessories
                            : [];

                        setFormData({
                          ...formData,
                          device_id: selectedId,
                          accessory_id: defaultAccessories,
                        });
                      }}
                      className={selectClass}
                    >
            ...
            {/* STEP 6 - NOTES + PHOTOS */}
            <section className={cardClass}>
              <div className="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2 lg:grid-cols-[0.9fr_1.1fr]">
                <div className="space-y-5">
                  {/* Transaction Notes */}
                  <div>
                    <label className={labelClass}>Transactions Notes</label>
